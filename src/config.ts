...
export function validateCampaign(input: unknown, index = 0): Campaign {
  ...
  if (c.keyword_mode !== undefined && c.keyword_mode !== "any" && c.keyword_mode !== "all" && c.keyword_mode !== "none") {
    throw new ConfigError(`${where}.keyword_mode must be "any", "all", or "none"`);
  }
  // In "none" mode (any comment triggers), keywords are optional.
  if (c.keyword_mode === "none") {
    if (c.keywords !== undefined && !Array.isArray(c.keywords)) {
      throw new ConfigError(`${where}.keywords must be an array when provided`);
    }
  } else {
  ...
  return {
    ...
    keyword_mode: (c.keyword_mode === "all" || c.keyword_mode === "none") ? c.keyword_mode : "any",
