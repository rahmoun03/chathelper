...
function paintAutoList() {
  ...
  rows.forEach((c) => {
    ...
    const kwMode = c.keyword_mode || "any";
    const kwPills = kwMode === "none"
      ? `<span class="kw-pill">Any word</span>`
      : (c.keywords || []).map((k) => `<span class="kw-pill">${esc(k)}</span>`).join("") || `<span class="kw-pill">—</span>`;
...
function paintArchiveList() {
  ...
  rows.forEach((c) => {
    const kwMode2 = c.keyword_mode || "any";
    const kwPills2 = kwMode2 === "none"
      ? `<span class="kw-pill">Any word</span>`
      : (c.keywords || []).map((k) => `<span class="kw-pill">${esc(k)}</span>`).join("") || `<span class="kw-pill">—</span>`;
    const row = el(`
      <div class="auto-row archived-row" data-id="${esc(c.campaign_id)}">
        ...
        <div class="auto-name-cell">
          ...
          <div class="auto-trigger"><span>User comments and comment contains</span>${kwPills2}</div>
        </div>
        <div></div>
...
function draftFromCampaign(c) {
  return {
    ...
    keywords: c.keywords || [],
    keyword_mode: c.keyword_mode || "any",
    exclude: c.exclude || [],
...
function renderSections() {
  ...
  const seeMoreBtn = hasMore
    ? `<button class="btn ghost sm" id="mediaSeeMore" style="margin-top:10px">${store.mediaShowAll ? "Show less" : `See more (${store.media.length - MEDIA_ROW_SIZE})`
    }</button>`
    : "";
  ...
  box.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="radio-row ${d.keyword_mode !== "none" ? "selected" : ""}" id="kwModeSpecific"><span class="radio-dot"></span><div><div class="rr-title">A specific word or words</div></div></div>
      <div id="kw_fields_wrap" style="${d.keyword_mode === "none" ? "display:none" : ""}">
      ...
      <div class="kw-mode-row" style="margin-top:10px;display:flex;gap:16px;align-items:center">
        <span class="label" style="font-size:13px;color:var(--muted)">Match mode:</span>
        <label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:13px">
          <input type="radio" name="kw_match_mode" value="any" ${d.keyword_mode === "any" || !d.keyword_mode ? "checked" : ""}/> Any keyword</label>
        <label style="display:flex;align-items:center;gap:5px;cursor:pointer;font-size:13px">
          <input type="radio" name="kw_match_mode" value="all" ${d.keyword_mode === "all" ? "checked" : ""}/> All keywords</label>
      </div>
      <div class="hint" style="margin-top:4px">${d.keyword_mode === "all" ? "Comment must contain <b>every</b> keyword to trigger." : "Comment triggers if it contains <b>any one</b> of the keywords."}</div>
      ...
      <div class="radio-row ${d.keyword_mode === "none" ? "selected" : ""}" id="kwModeAny"><span class="radio-dot"></span><div><div class="rr-title">Any word</div><div class="rr-sub">Any comment on this post will trigger the DM — no keyword needed.</div></div></div>
...
function wireSections() {
  ...
  // keyword mode radios (any-keyword / all-keywords within "specific" mode)
  document.querySelectorAll('input[name="kw_match_mode"]').forEach((r) => {
    r.onchange = () => { d.keyword_mode = r.value; renderSections(); };
  });
  // top-level mode: specific words vs any word
  const kwSpecific = $("#kwModeSpecific");
  const kwAny = $("#kwModeAny");
  if (kwSpecific) kwSpecific.onclick = () => {
    if (d.keyword_mode === "none") { d.keyword_mode = "any"; renderSections(); }
  };
  if (kwAny) kwAny.onclick = () => {
    if (d.keyword_mode !== "none") { d.keyword_mode = "none"; renderSections(); }
  };
...
function renderPreview() {
  ...
  if (store.previewTab === "post") {
    body = `<div class="ig">
      ...
      <div class="ig-caption"><span class="u">${esc(uname)}</span>${esc(captionShown)}${captionLong ? `<span class="ig-caption-toggle" id="captoggle"> ${store.captionExpanded ? "less" : "more"}</span>` : ""
      }</div>
    </div>`;
...
function buildCampaignFromDraft() {
  ...
  return {
    ...
    name: d.name,
    media_id: d.media_id,
    keywords: d.keyword_mode === "none" ? [] : d.keywords,
    keyword_mode: d.keyword_mode || "any",
    exclude: d.exclude,
...
function validateDraft(d) {
  ...
  if (d.keyword_mode !== "none" && !d.keywords.length) return "Add at least one keyword in section 2.";
